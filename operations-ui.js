(() => {
  ...
  function summarize(rows){
    let revenue=0,collected=0,outstanding=0,cost=0,profit=0,costable=0,conditional=0,actual=0;
    rows.forEach(e=>{const f=fin(e),p=pay(e);revenue+=f.revenue;collected+=p.collected;outstanding+=p.outstanding;if(f.total!=null){cost+=f.total;costable++}if(f.profit!=null&&f.total!=null)profit+=f.profit;if(f.confidence==='estimated')conditional++;if(f.confidence==='actual')actual++});
    const coverage=rows.length?costable/rows.length*100:0;
    return{revenue,collected,outstanding,cost,profit,costable,conditional,actual,coverage,count:rows.length};
  }
  ...
  function actionQueue(){
    const actions=[];future().forEach(e=>{const f=fin(e),p=pay(e),fill=e.capacity?e.people/e.capacity*100:100;if(!f.recipe)actions.push([1,e.date,`${e.menu} 레시피 연결`,'원가·예상이익 계산 불가']);else if(f.total==null)actions.push([2,e.date,`${e.menu} 원가 보완`,f.costStatus||'미산정']);if(p.outstanding>0)actions.push([3,e.date,`${e.menu} 미수금 확인`,`${won(p.outstanding)} 미입금`]);if(fill<70)actions.push([4,e.date,`${e.menu} 모집 보강`,`충원 ${pct(fill)} · ${Math.max(0,e.capacity-e.people)}석 남음`])});
  ...
  function renderDashboard(){
    ...
    host.innerHTML=`<section class="decision-block"><div class="decision-head"><div><h3>이번 달 결정 요약</h3><p>완료 수업과 예약 수업을 현재 데이터 기준으로 합산합니다.</p></div><span class="confidence ${cf[1]}">원가 신뢰도 ${cf[0]}</span></div><div class="decision-stats">${stat('수업 매출',won(s.revenue),`${s.count}개 수업`)}${stat('입금 기록',won(s.collected),'참가자 결제 데이터','good')}${stat('미수금',won(s.outstanding),'추가 확인 필요',s.outstanding?'warn':'')}${stat('계산가능 비용',s.costable?won(s.cost):'계산 보류',`${s.costable}/${s.count} 수업`)}${stat('추정이익',s.costable?won(s.profit):'계산 보류',`원가 커버리지 ${pct(s.coverage)}`,s.coverage<70?'bad':'')}${stat('다음달 예약매출',won(ns.revenue),`${ns.count}개 예정`)}</div></section><div class="decision-grid"><section class="decision-block"><div class="decision-head"><div><h3>다음 행동</h3><p>원가·입금·모집 상태에서 바로 처리할 항목</p></div></div><div class="action-list">${actions.length?actions.map(x=>`<div class="action-row"><time>${esc(x[1].slice(5).replace('-','.'))}</time><div><b>${esc(x[2])}</b><small>${esc(x[3])}</small></div></div>`).join(''):'<div class="decision-empty">우선 처리할 항목이 없습니다.</div>'}</div></section><section class="decision-block"><div class="decision-head"><div><h3>가까운 수업</h3><p>충원 · 입금 · 예상이익</p></div></div><div class="next-list">${up.length?up.map(e=>{const f=fin(e),p=pay(e),fill=e.capacity?e.people/e.capacity*100:0;return `<button class="next-decision-row" data-ops-index="${e.index}"><time>${esc(e.date.slice(5).replace('-','.'))} ${esc(e.time)}</time><b>${esc(e.menu)}</b><span>충원 ${pct(fill)} · 입금 ${pct(p.rate)} · ${f.profit==null?'이익 계산 보류':`${esc(f.profitLabel)} ${won(f.profit)}`}</span></button>`}).join(''):'<div class="decision-empty">예정 수업이 없습니다.</div>'}</div></section></div><section class="decision-block"><div class="decision-head"><div><h3>6개월 매출 · 입금 흐름</h3><p>매출 기준액과 실제 입금 기록을 분리합니다.</p></div></div>${cashChart()}</section>`;
  ...
  function renderRecipes(){
    const page=$('recipes');if(!page)return;const list=Array.isArray(recipes)?recipes:[],confirmed=list.filter(r=>B.costState(r).confidence==='confirmed').length,conditional=list.filter(r=>B.costState(r).confidence==='estimated').length,incomplete=list.filter(r=>!B.costState(r).usable).length,rec=D?D.reconciliation(list,ingredients,5):null,prov=D?D.provenanceAudit(ingredients):null;
    ...
    host.innerHTML=`<div class="ops-summary-strip">${stat('확정 원가',`${confirmed}개`,'확정 단가 기준','good')}${stat('조건부 원가',`${conditional}개`,'추정값으로 사용',conditional?'warn':'')}${stat('원가 미완료',`${incomplete}개`,'이익 계산 제외',incomplete?'bad':'')}${stat('재계산 완료',rec?`${rec.complete}/${rec.total}`:'—','공통 단가 기준')}${stat('원가 차이 ≥5%',rec?`${variance.length}개`:'—','저장 원가 검토',variance.length?'warn':'')}${stat('구매가 재확인',prov?`${review.length}개`:'—','쿠팡/지정제품 provenance',review.length?'warn':'')}</div><div class="recipe-review-list">${variance.length||review.length?`<b>검토 우선:</b> ${esc([...new Set([...variance.map(x=>x.name),...review])].slice(0,8).join(', '))}`:'현재 자동 검증에서 큰 경고가 없습니다.'}</div>`;
  ...
  function menuPerformance(rows){
    const map=new Map();rows.forEach(e=>{const f=fin(e);if(!map.has(e.menu))map.set(e.menu,{menu:e.menu,classes:0,people:0,capacity:0,revenue:0,profit:0,costable:0});const x=map.get(e.menu);x.classes++;x.people+=e.people;x.capacity+=e.capacity;x.revenue+=f.revenue;if(f.profit!=null&&f.total!=null){x.profit+=f.profit;x.costable++}});
  ...
  function renderFinance(){
    const page=$('finance');if(!page)return;const [a,b]=periodRange(),rows=range(a,b),s=summarize(rows),cf=confidence(s),menus=menuPerformance(rows),nm=nextMonth(todayISO()),next=summarize(range(nm,monthEnd(nm)).filter(e=>e.source==='schedule')),nf=confidence(next),base=s.revenue||1;
    ...
    host.innerHTML=`<section class="decision-block forecast"><div class="decision-head"><div><h3>${Number(nm.slice(5,7))}월 예상 수익</h3><p>현재 등록된 다음 달 일정과 레시피 원가 기준</p></div><span class="confidence ${nf[1]}">원가 커버리지 ${pct(next.coverage)}</span></div><div class="decision-stats">${stat('예정 수업',`${next.count}회`,'취소 제외')}${stat('예약매출',won(next.revenue),'현재 예약 인원 기준')}${stat('계산가능 비용',next.costable?won(next.cost):'계산 보류',`${next.costable}/${next.count} 수업`)}${stat('예상 순이익',next.costable?won(next.profit):'계산 보류',`${next.conditional}개 조건부 포함`,next.coverage<70?'warn':'')}${stat('입금 기록',won(next.collected),'현재 결제 데이터')}${stat('미수금',won(next.outstanding),'추가 입금 필요',next.outstanding?'warn':'')}</div></section><section class="decision-block"><div class="decision-head"><div><h3>기간 수익 요약</h3><p>${esc(a)} ~ ${esc(b)} · 매출과 현금을 분리합니다.</p></div><span class="confidence ${cf[1]}">원가 신뢰도 ${cf[0]} · ${pct(s.coverage)}</span></div><div class="decision-stats finance">${stat('수업 매출',won(s.revenue),`${s.count}개 수업`)}${stat('입금 기록',won(s.collected),'참가자 결제 데이터','good')}${stat('미수금',won(s.outstanding),'매출 기준액 - 입금',s.outstanding?'warn':'')}${stat('계산가능 비용',s.costable?won(s.cost):'계산 보류',`${s.costable}/${s.count} 수업`)}${stat('추정이익',s.costable?won(s.profit):'계산 보류',`${s.conditional}개 조건부 포함`,s.coverage<70?'bad':'')}${stat('원가 커버리지',pct(s.coverage),'계산 가능한 수업 비율')}</div></section><section class="decision-block"><div class="decision-head"><div><h3>매출에서 남는 구조</h3><p>원가 계산 가능한 수업만 포함합니다.</p></div></div><div class="profit-flow"><div><span>비용</span><i style="width:${Math.min(100,Math.max(0,s.cost/base*100))}%"></i><b>${s.costable?won(s.cost):'—'}</b></div><div><span>추정이익</span><i class="profit" style="width:${Math.min(100,Math.max(0,s.profit/base*100))}%"></i><b>${s.costable?won(s.profit):'—'}</b></div></div></section><section class="decision-block"><div class="decision-head"><div><h3>메뉴 성과</h3><p>평균 충원율과 수업당 이익으로 비교합니다.</p></div></div><div class="tablewrap"><table class="decision-table"><thead><tr><th>메뉴</th><th>수업</th><th>평균 충원</th><th>수업당 이익</th><th>마진</th></tr></thead><tbody>${menus.length?menus.map(x=>`<tr><td><b>${esc(x.menu)}</b></td><td>${x.classes}회</td><td>${pct(x.fill)}</td><td>${x.profitPerClass==null?'계산 보류':won(x.profitPerClass)}</td><td>${pct(x.margin)}</td></tr>`).join(''):'<tr><td colspan="5">데이터가 없습니다.</td></tr>'}</tbody></table></div></section>`;
