(() => {
  ...
  function openRecipe(name){
    ...
    setTimeout(()=>{const card=[...document.querySelectorAll('#recipeList details.recipe')].find(c=>c.querySelector('.rname')?.textContent.trim()===name);if(!card)return;
      ...
      setTimeout(()=>{const input=[...card.querySelectorAll('[data-rk="cost"]')].find(x=>x.dataset.r===name)||card.querySelector('[data-rk="cost"]');if(input){input.focus({preventScroll:true});input.select?.()}setTimeout(()=>card.classList.remove('ux-cost-target'),1600)},220);
    },260);
  }
  ...
  function renderQueues(){
    ...
    if(dash&&items.length){if(!d){d=document.createElement('section');d.id='uxCostReviewDashboard';d.className='decision-block ux-cost-review';const a=dash.querySelector('.lean-signal-grid');a?a.insertAdjacentElement('afterend',d):dash.prepend(d)}const shown=items.slice(0,4);d.innerHTML=`<div class="ux-review-head"><div><h3>원가 확인할 레시피</h3><p>확인이 끝나면 수익 계산에 바로 반영됩니다.</p></div><b>${items.length}개</b></div><div class="ux-review-list">${shown.map(x=>item(x,true)).join('')}</div>${items.length>4?'<button type="button" class="ux-review-more" data-ux-open-costs>전체 보기</button>':''}`}
    else d?.remove();
    const tools=document.querySelector('#recipes .recipe-tools');let r=document.getElementById('uxRecipeCostReview');
    if(tools&&items.length){if(!r){r=document.createElement('section');r.id='uxRecipeCostReview';r.className='ux-recipe-review'}tools.insertAdjacentElement('afterend',r);r.innerHTML=`<div class="ux-review-head"><div><h3>원가 확인 필요</h3><p>누르면 해당 레시피의 원가 입력칸으로 바로 이동합니다.</p></div><b>${items.length}개</b></div><div class="ux-review-list recipes">${items.map(x=>item(x,true)).join('')}</div>`}else r?.remove();
  }

  function setLabel(el,text){if(!el)return;const n=[...el.childNodes].find(x=>x.nodeType===Node.TEXT_NODE&&x.nodeValue.trim());if(n)n.nodeValue=text;else el.prepend(document.createTextNode(text))}
  function navCopy(){
    ...
    const top=document.querySelector('.top');if(top){const h=top.querySelector('h1'),p=top.querySelector('p');if(h)h.textContent='BLEU 운영';if(p)p.textContent='수업 · 레시피 · 원가 · 수익을 한곳에서 관리합니다.'}
    const cal=document.querySelector('#calendar .section-head');if(cal){cal.querySelector('h2').textContent='수업';cal.querySelector('p').textContent='날짜를 눌러 수업과 참가자, 입금 상태를 관리합니다.'}
    const rp=document.querySelector('#recipes .section-head p');if(rp)rp.textContent='레시피와 재료 원가를 확인하고 필요한 항목만 수정합니다.';
    const fin=document.querySelector('#finance .section-head');if(fin){fin.querySelector('h2').textContent='수익';fin.querySelector('p').textContent='이번 달 남는 금액과 월말 예상부터 확인합니다.'}
    const s=document.querySelector('#leanDashboard .lean-signal:nth-child(4)');if(s){s.querySelector('span').textContent='수익 계산 보류';s.querySelector('small').textContent='원가 확인 필요한 수업'}
  }

  function recipeCopy(){
    document.querySelectorAll('#recipeList [data-rk="status"]').forEach(sel=>{[...sel.options].forEach(o=>{const v=o.getAttribute('value')||o.value||o.textContent.trim();if(['확정','조건부','부분원가','미산정'].includes(v)){o.value=v;o.textContent=label(v)}});setLabel(sel.closest('label'),'확인 상태')});
    document.querySelectorAll('#recipeList [data-rk="cost"]').forEach(x=>setLabel(x.closest('label'),'완성 원가'));
    document.querySelectorAll('#recipeList [data-action="recipe-save"]').forEach(x=>x.textContent='저장');
  ...
  function classCopy(){const body=document.getElementById('dayOpsBody');if(!body)return;setLabel(body.querySelector('label:has([data-core="people"])'),'예약 인원');setLabel(body.querySelector('label:has([data-core="capacity"])'),'정원');setLabel(body.querySelector('label:has([data-core="fee"])'),'1인 수강료');setLabel(body.querySelector('label:has(#dayOpsPeople)'),'예약 인원');const add=document.getElementById('dayOpsAddClass');if(add)add.textContent='수업 추가'}
  ...
  function financeCopy(){
    const host=document.getElementById('financeLedgerDetail');if(!host)return;const intro=host.querySelector('.finance-focus-intro');if(intro){intro.querySelector('h2').textContent='이번 달 수익';intro.querySelector('p').textContent='지금까지 남은 금액과 현재 예약 기준 월말 예상을 봅니다.'}
    const a=host.querySelector('.focus-flow-card.current .focus-flow-head h3'),b=host.querySelector('.focus-flow-card.forecast .focus-flow-head h3');if(a)a.textContent='지금까지';if(b)b.textContent='월말 예상';host.querySelectorAll('.focus-flow-badge').forEach(x=>x.textContent=x.textContent.replace('원가 연결','원가 확인').replace(/\s*·\s*\d+%/,''));
    const bad=blockers();let alert=host.querySelector('.ux-finance-alert');if(!alert){alert=document.createElement('button');alert.type='button';alert.className='ux-finance-alert';alert.dataset.uxFinanceCosts='';(host.querySelector('.finance-supercard')||host.querySelector('.finance-focus-grid'))?.insertAdjacentElement('afterend',alert)}if(alert){alert.hidden=!bad.length;alert.innerHTML=bad.length?`<span>원가 때문에 계산 안 되는 수업</span><b>${bad.length}건</b><em>확인 →</em>`:''}
  ...
  ['renderDashboard','renderRecipes','renderFinance','renderAll'].forEach(k=>{try{const base=window[k];if(typeof base==='function')window[k]=function(...args){const out=base.apply(this,args);setTimeout(refresh,190);return out}}catch(e){}});
  document.addEventListener('click',e=>{const r=e.target.closest('[data-ux-cost-recipe]');if(r){e.preventDefault();openRecipe(r.dataset.uxCostRecipe);return}if(e.target.closest('[data-ux-open-costs],[data-ux-finance-costs]')){go('recipes');setTimeout(()=>document.getElementById('uxRecipeCostReview')?.scrollIntoView({behavior:'smooth',block:'start'}),190);return}setTimeout(refresh,180)},true);
  document.addEventListener('input',()=>setTimeout(refresh,180),true);document.addEventListener('change',()=>setTimeout(refresh,180),true);document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')setTimeout(refresh,200)});
