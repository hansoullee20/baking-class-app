(() => {
  ...
  function renderTable(recipe,table){
    ...
    if(summary)summary.innerHTML=`<div><span>이번 배합 실사용 재료비</span><b>${linked.length?won(sum):'—'}</b><small>${linked.length}/${rows.length}개 재료 계산 가능 · 각 재료의 실제 용량 × 현재 단가 합계</small></div>`;
  }
